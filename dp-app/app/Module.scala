...
import com.hortonworks.dataplane.cs.Webservice.{AmbariWebService, ClusterClient, ConfigurationUtilityService}
...
import services.{ConsulHealthService, PluginManifestService, SslContextManager}
...
class Module extends AbstractModule {
  ...
  @Provides
  @Singleton
  @Named("pluginManifestService")
  def providePluginManifestService(configuration: Configuration): PluginManifestService = {
    new PluginManifestService(configuration.underlying)
  }
  ...
  @Singleton
  @Named("clusterClient")
  def provideClusterClient(implicit ws: WSClient,configuration: Configuration):ClusterClient = {
    implicit val clusterWsClient:ClusterWsClient = ClusterWsClient(ws)
    new ClusterClientImpl(configuration.underlying)
  }
  ...
  @Provides
  @Singleton
  @Named("clusterSkuService")
  def provideClusterSkuService(implicit ws: WSClient,configuration: Configuration): ClusterSkuService = {
    new ClusterSkuServiceImpl(configuration.underlying)
  }
