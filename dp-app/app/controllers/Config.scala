...
class Config @Inject()(
  ...
  def init = AuthenticatedAction.async { request =>
    for {
      ...
      Json.obj(
        ...
        "version" -> appConfiguration.getString("dp.version").getOrElse("1.2").toString
      )
    )
  }
  ...
  def getDPSDetails() = Action.async {
    Future.successful(Ok(Json.obj("version" -> appConfiguration.getString("dp.version").getOrElse("1.2").toString)))
  }
