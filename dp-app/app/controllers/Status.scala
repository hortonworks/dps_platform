...
@Singleton
class Status @Inject()(metricsRegistry: MetricsRegistry)
    extends Controller {
  ...
  def metrics = Action.async { req =>
    ...
    val exported = req.queryString.getOrElse(MetricsReporter.exportParam, Seq())
    if (exported.nonEmpty && exported.head == MetricsReporter.prometheus) {
