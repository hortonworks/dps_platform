...
class KnoxConfigurator @Inject()(private val config: Configuration) {
  ...
  def configure() = {
    val event: ConsulEvent =
      new ConsulEvent("knoxSSoToplogyConfigured", null, "reconfigure_knox")
    val eventResp = dpConsulClient.fireEvent(event)
    logger.info(s"Fired knoxSsoTopolgyConfigured ConsulEvent id=(${eventResp.getId}) at time : ${eventResp.getlTime()}")
  }
}
