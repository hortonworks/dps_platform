...
export class LdapConfigCommonComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    .subscribe(({certs, config}) => {
      this.ldapProperties = Object.keys(config).length > 0 ? config : new LDAPProperties();
      if(certs.length > 0) {
        this.cert = certs[0];
      }
      this.ldapProperties = config;
      let domain = window.location.hostname;
      if(!this.ldapProperties.domains){
        this.ldapProperties.domains = [];
        this.ldapProperties.domains.push(domain);
      }else if(!this.ldapProperties.domains.find(dm => dm === domain)) {
        this.ldapProperties.domains.push(domain);
        if (certs.length > 0) {
          this.cert = certs[0];
        }
  ...
  setDefaultObjectClassValue(){
    if(this.ldapProperties && !this.ldapProperties.userObjectClass){
      ...
      this.ldapProperties.useAnonymous = false;
      this.ldapProperties.referral = 'ignore';
    }
  }
  ...
  toggleReferral() {
    if(this.ldapProperties.referral === 'follow') {
      this.ldapProperties.referral = 'ignore';
    } else {
      this.ldapProperties.referral = 'follow';
    }
  }
