...
export class CertUploadComponent {
  ...
  upload(): void {
    ...
    if(!this.uploadForm.valid || !files || !files.length){
      this.showError = true;
      this.errorMessage =  this.translateService.instant("pages.settings.description.validationErrorMessage");
      return;
    }
    let file = files[0];
    if(!/^[a-z0-9_-]+$/i.test(this.name)){
      this.showError = true;
      this.errorMessage = this.translateService.instant("pages.settings.description.invalidCertName");
      return;
    }else if(this.name.length > 255){
      this.showError =true;
      this.errorMessage = this.translateService.instant("pages.settings.description.certNameLengthExceeds");
      return;
    } else if(file.size > 50*1024){
      this.showError = true;
      this.errorMessage = this.translateService.instant("pages.settings.description.certFileSizeExceeds");
      return;
    }
    this.uploading = true;
