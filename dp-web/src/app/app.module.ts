...
export class DPSMissingTranslationHandler implements MissingTranslationHandler {
  handle(params: MissingTranslationHandlerParams) {
    if(params.key.startsWith('common.roles.') && params.key.trim().split('.').length === 3) {
      return params.key.trim().split('.')[2];
    }
    return params.key;
  }
}
  ...
  imports: [
    ...
    TranslateModule.forRoot({
      missingTranslationHandler: {
        ...
        useClass: DPSMissingTranslationHandler
      },
      loader: {
