...
export class AuthenticationService {
  ...
  getAuthServerStatus(url) {
    return this.httpClient.get(url, {responseType: 'text'});
  }

  signIn(credential: Credential): Observable<User> {
    return this.httpClient.post<User>(`${this.URI}/in`, credential).do((user: User) => {
  ...
  signOut(): Observable<any> {
    return this.httpClient.get(`${this.URI}/out`, {observe: 'response'})
      .map(response => {
        const challengeAt = response.headers.get(HEADER_CHALLENGE_HREF);
        return (challengeAt && challengeAt.length > 0 && typeof challengeAt === 'string') ? challengeAt : '';
      });
  }
