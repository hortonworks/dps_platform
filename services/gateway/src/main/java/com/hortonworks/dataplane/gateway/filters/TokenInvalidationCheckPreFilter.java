...
public class TokenInvalidationCheckPreFilter extends ZuulFilter {
  ...
  @Autowired
  private BlacklistCache tokenService;
  ...
  public Object run() {
    ...
    if(tokenService.isBacklisted(token) || tokenService.isMarkedForRefresh(jwt.parseJWT(token).getUsername(), jwt.getExpiration(token).toInstant())) {
      throw new GatewayException(HttpStatus.UNAUTHORIZED, "This token has been marked as invalid. Please login again.");
    }
