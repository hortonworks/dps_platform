...
class DpClusters @Inject()(dpClusterRepo: DpClusterRepo)(
  ...
  def all = Action.async { request =>
    if(request.getQueryString("clusterName").isDefined && request.getQueryString("datacenterName").isDefined) {
      val clusterName = request.getQueryString("clusterName").get
      val datacenterName = request.getQueryString("datacenterName").get
      dpClusterRepo
        .findByNameAndDatacenter(clusterName, datacenterName)
  ...
  def deleteByParams(clusterName: Option[String], datacenterName: Option[String]) = Action.async { req =>
    if(clusterName.isDefined && datacenterName.isDefined) {
      val future = dpClusterRepo.deleteByNameAndDatacenter(clusterName.get, datacenterName.get)
      future.map(i => success(true)).recoverWith(apiError)
    } else {
      apiError.apply(new Exception(s"'clusterName' and 'datacenterName' query parameters are required."))
    }
  }
