...
class ClusterServiceRepo @Inject()(
  ...
  def upsertByName(clusterId: Long, serviceName: String, cs: ClusterService): Future[ClusterService] = db.run {
    (for {
      updated <- Services
        .filter(cService => cService.clusterid === clusterId && cService.servicename === serviceName)
        .map(r => r.properties)
        .update(cs.properties)

      cs <-  updated match {
        case 0 => Services returning Services += cs
        case _ => Services.filter(cService => cService.clusterid === clusterId && cService.servicename === serviceName).result.head
      }
    } yield cs).transactionally
  }
