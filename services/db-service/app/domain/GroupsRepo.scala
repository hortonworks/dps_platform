...
class GroupsRepo @Inject()(protected val dbConfigProvider: DatabaseConfigProvider,
                           protected val roleRepo: RoleRepo) extends HasDatabaseConfigProvider[DpPgProfile] {
          ...
          roleRepo.all().map { allRoles =>
            groups.map { group =>
              val grpRoles = roleIdUsersMap.get(group.id.get) match {
                case Some(roles) => allRoles.filter(cRole => roles.contains(cRole.id.get)).map(_.roleName)
                case None => Seq()
              }
  ...
  def addGroupWithRoles(groupInfo: GroupInfo) = {
    ...
    roleRepo.all().flatMap { allRoles =>
      val query = for {
        group <- Groups returning Groups += group
        groupRoles <- {
          val groupRolesList = allRoles
            .filter(cRole => groupInfo.roles.contains(cRole.roleName))
            .map(cRole => GroupRole(groupId=group.id, roleId = cRole.id))
  ...
  def getGroupByName(groupName: String):Future[GroupInfo] = {
    for {
      ...
      val roles = groupRoles.map{ groupRoleObj=> allRoles.find(cRole => cRole.id.get == groupRoleObj._2.get.roleId.get).get.roleName }
  ...
  def getRolesForGroups(groupIds:Seq[Long])={
    ...
    db.run(query).flatMap{res=>
      ...
      roleRepo.all().map { _.filter(cRole => roleIds.contains(cRole.id.get)).map(_.roleName) }
    }
  }
  ...
  private def resolveGroupRolesEntries(roles: Seq[String], groupRoles: Seq[GroupRole]): Future[(Seq[Long], Seq[Long])] = {
    roleRepo.all().map { allRoles =>
      val requiredRoleIds: Seq[Long] = roles.map(roleName => allRoles.find(_.roleName == roleName).get.id.get)
  ...
  def updateGroupInfo(groupInfo: GroupInfo) = {
    for {
      ...
      val groupRoleObjs = toBeAddedRoleIds.map(roleId => GroupRole(groupId = Some(group.id.get), roleId = Some(roleId)))
      val query = for {
