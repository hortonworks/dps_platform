...
class UserRepo @Inject()(protected val dbConfigProvider: DatabaseConfigProvider,
                         ...
                         protected val groupsRepo:GroupsRepo) extends HasDatabaseConfigProvider[DpPgProfile] {
  ...
  def getUserDetail(userName:String):Future[UserInfo]={
    for {
      ...
      val roles= userRoles.map{userRoleObj=> allRoles.find(cRole => cRole.id.get == userRoleObj.roleId.get).get.roleName }
  ...
  def updateUserAndRoles(userInfo:UserInfo, groupManaged:Boolean):Future[UserInfo]={
    val queryFuture=for{
      ...
      val toBeAddedRoleObjs = toBeAddedRoleIds.map(roleId => UserRole(userId=Some(user.id.get),roleId = Some(roleId)))
      for{
        updateUser <- {
  ...
  def insertUserWithRoles(userInfo:UserInfo,password:String):Future[UserInfo]={
    ...
    roleRepo.all().flatMap { allRoles =>
      val query = for{
        ...
        userRoles <-{
          val userRoleObjs = allRoles
            .filter(cRole => userInfo.roles.contains(cRole.roleName))
            .map(cRole => UserRole(userId=user.id, roleId = cRole.id))
  ...
  private def resolveUserRolesEntries(roles: Seq[String], userRoles: Seq[UserRole]): Future[(Seq[Long],Seq[Long])] = {
    roleRepo.all().map { allRoles =>
