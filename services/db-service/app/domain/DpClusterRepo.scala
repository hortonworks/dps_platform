...
import com.hortonworks.dataplane.commons.domain.Entities.{CategoryCount, DataplaneCluster, Location}
...
class DpClusterRepo @Inject()(
    ...
    protected val clusterSkuRepo: ClusterSkuRepo,
  ...
  def findByNameAndDatacenter(dpClusterName: String, datacenterName: String): Future[Option[DataplaneCluster]] = {
    val clName = dpClusterName.toLowerCase
    val dcName = datacenterName.toLowerCase
    db.run(DataplaneClusters.filter( x => x.name.toLowerCase === clName && x.dcName.toLowerCase === dcName).result.headOption)
  }
...
//  TODO: ambari ip address actually is the complete url, just with domain replaced with ip; replace it later to point to ambariUrl
  ...
  def deleteByNameAndDatacenter(clusterName: String, datacenterName: String): Future[Int] = {
    for {
      clusterOption <- findByNameAndDatacenter(clusterName, datacenterName)
      cluster = clusterOption.get
      deletedCount <- deleteCluster(cluster.id.get)
    } yield deletedCount.head
  }
