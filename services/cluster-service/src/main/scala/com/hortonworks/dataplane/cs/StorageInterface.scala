...
import com.hortonworks.dataplane.commons.domain.Entities.{Cluster, ClusterHost, ClusterServiceHost, DataplaneCluster, Errors, WrappedErrorException, ClusterService => ClusterData}
...
trait StorageInterface {
  ...
  def updateServiceByName(toPersist: ClusterData, hosts: Seq[ClusterServiceHost]): Future[Boolean]

  def upsertServiceByName(toPersist: ClusterData, hosts: Seq[ClusterServiceHost]): Future[Boolean]
  ...
  def addService(service: ClusterData, hosts: Seq[ClusterServiceHost]): Future[Option[ClusterData]]
...
class StorageInterfaceImpl @Inject()(
  ...
  def mapHostsToCluster(s: Either[Errors, ClusterData],
    ...
    Future.successful {
      if (s.isLeft) {
        throw new Exception(s"Could not create the service entry ${s.left.get}")
      } else {
  ...
  override def upsertServiceByName(toPersist: ClusterData, hosts: Seq[ClusterServiceHost]): Future[Boolean] = {
    for {
      cs <- clusterComponentService.upsertServiceByName(toPersist.clusterId.get.toString, toPersist.serviceName, toPersist)
      endpointUpdate <- clusterComponentService.updateClusterHosts(hosts.map(_.copy(serviceid = cs.id)))
    } yield {
      val updateResult = endpointUpdate.foldRight(true)((a, b) => a.isRight && b)

      endpointUpdate.foreach {
        case Left(errors) => logger.error(s"Service endpoint update errors $errors")
        case Right(result) => logger.info(s"Service endpoint update result - $result")
      }
      updateResult
    }
  }
