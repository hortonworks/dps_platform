    ...
    ws.url(endpoint)
      ...
      .map { response =>
        response.status match {
          ...
          case 302 => {
            // Trusted proxy response from Knox
            logger.info(s"Handling a 302 response likely from Knox working with Ambari in Trusted Proxy mode")
            response.header(HttpHeaders.LOCATION) match {
              case Some(location) => {
                val jwtProviderUrl = location.split("\\?")(0)
                logger.info(s"Extracting Knox provider URL as $jwtProviderUrl")
                Some(jwtProviderUrl)
              }
              case _ => throw WrappedErrorException(Error(500, s"No location header given in response for $endpoint", "cluster.knox.status.missing-location-header"))
            }
