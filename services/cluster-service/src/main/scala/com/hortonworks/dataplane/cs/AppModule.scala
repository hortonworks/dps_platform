...
object AppModule extends AbstractModule {
  ...
  @Provides
  @Singleton
  def provideClusterRoute(clusterDataApi: ClusterDataApi, config: Config): ClusterRoute = {
    new ClusterRoute(clusterDataApi, config)
  }
  ...
  def provideWebservice(actorSystem: ActorSystem,
    ...
    new Webserver(
      ...
      rangerRoute.rangerAudit ~
        ...
        dpProfilerRoute.updateProfilerSelector ~
        dpProfilerRoute.getProfilerSelectorsConfig ~
        ...
        clusterRoute.clusterKnoxTokenProxy ~
