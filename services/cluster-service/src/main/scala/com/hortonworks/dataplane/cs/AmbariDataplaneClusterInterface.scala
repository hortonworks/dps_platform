...
sealed trait AmbariDataplaneClusterInterface {
  ...
  def getHdpVersion(clusterName:String)(implicit hJwtToken: Option[HJwtToken]): Future[Seq[String]]
  ...
  def getStackDisplayName(clusterName: String)(implicit  hJwtToken: Option[HJwtToken]): Future[Option[String]]
...
class AmbariDataplaneClusterInterfaceImpl(dataplaneCluster: DataplaneCluster,
  ...
  override def getHdpVersion(clusterName:String)(implicit hJwtToken: Option[HJwtToken]): Future[Seq[String]] = {
    val url = s"${dataplaneCluster.ambariUrl}$prefix/$clusterName?fields=Clusters/version"
    val response = getAmbariResponse(url, dataplaneCluster.allowUntrusted)

    response.map { res =>
      val version = (res.json \ "Clusters" \ "version").validate[String].getOrElse("")
      Seq(version)
    }
  }
  ...
  override def getStackDisplayName(clusterName: String)(implicit  hJwtToken: Option[HJwtToken]): Future[Option[String]] = {
    val url = s"${dataplaneCluster.ambariUrl}$prefix/$clusterName/stack_versions?ClusterStackVersions/state=CURRENT&fields=repository_versions/RepositoryVersions/display_name"
    val response = getAmbariResponse(url, dataplaneCluster.allowUntrusted)

    response.map { res =>
      (res.json \ "items" \\ "repository_versions")
        .map(_.as[Seq[JsValue]])
        .flatten  // flattening seq
        .map(stack => (stack \ "RepositoryVersions" \ "display_name").asOpt[String])
        .headOption
        .flatten  // flattening options
    }
  }
}
