...
class DpClusterSync @Inject()(val actorSystem: ActorSystem,
  ...
  def createClusterIfNotExists(
    ...
    val clusters = for {
      ...
      stackDisplayName <- interface.getStackDisplayName(clusterNames.head)
    } yield (clusterNames.head, clusterDetails, stackDisplayName)

    val toInsert = clusters.map { case (clusterName, clusterProperties, stackDisplayName) =>
      Cluster(
        name = clusterName,
        clusterUrl = Some(s"${dataplaneCluster.ambariUrl}$prefix/${clusterName}"),
        secured = extractSecurity(clusterProperties),
        properties = clusterProperties,
        dataplaneClusterId = dataplaneCluster.id,
        userid = dataplaneCluster.createdBy,
        version = stackDisplayName
      )
    }
