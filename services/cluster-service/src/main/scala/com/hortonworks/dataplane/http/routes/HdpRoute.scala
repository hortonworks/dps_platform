          ...
          for {
            ...
            parsedHeader <- {
              if (shouldUseKnoxSingleSignOn(jwtToken, knoxUrl)) {
                ...
                val accessToken = clusterData.getTokenForCluster(cluster.toLong, jwtToken)
                accessToken.map {
            ...
            h <- {
              ...
              val finalPath = if (shouldUseKnoxSingleSignOn(jwtToken, knoxUrl) && dpCluster.behindGateway && serviceEnabled(service)) s"${targetUrl.getPath}/$service/$targetPath" else s"${targetUrl.getPath}/$targetPath"
