...
class DpProfilerServiceImpl (val config: Config)(implicit ws: ClusterWsClient) extends DpProfilerService{
  ...
  override def getProfilerSelectorsConfig (clusterId: String) (implicit token:Option[HJwtToken]) : Future[JsObject] = {
    ws.url(s"$url/cluster/$clusterId/dp-profiler/selectors")
      .withToken(token)
      .withHeaders("Accept" -> "application/json")
      .get()
      .map(mapToResultsGeneric)
  ...
  override def updateProfilerSelector(clusterId: String, name: String, body: JsValue) (implicit token:Option[HJwtToken]) : Future[JsObject] = {
    ws.url(s"$url/cluster/$clusterId/dp-profiler/selectors/$name")
      .withToken(token)
      .withHeaders("Accept" -> "application/json")
      .put(body)
      .map(mapToResultsGeneric)
