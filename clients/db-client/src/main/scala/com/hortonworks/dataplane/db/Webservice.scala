...
object Webservice {
  ...
  @deprecated(since = "1.2.1.0", message = "Would be removed or made private in 2.0. Apps need to manage their own data.")
  ...
  trait DpClusterService extends DbClientService {
    ...
    def delete(dpClusterName: String, dpDatacenter: String): Future[Either[Errors, Boolean]]
  ...
  trait ClusterComponentService extends DbClientService {
    ...
    def listClusterServices(clusterId: String, serviceName: Option[String]): Future[Seq[ClusterData]]
    ...
    def listClusterServiceHosts(clusterId: String, serviceName: String): Future[Seq[String]]
    ...
    def upsertServiceByName(clusterId: String, serviceName: String, cs: ClusterData): Future[ClusterData]
  ...
  @deprecated(since = "1.2.1.0", message = "Would be removed or made private in 2.0. Apps need to manage their own data.")
  trait ClusterSkuService extends DbClientService {
    def getAllBySkuId(skuId: Long): Future[Seq[ClusterSku]]
    def getClusterSku(skuId: Long, dpClusterId: Long) : Future[Option[ClusterSku]]
    def enableSkuForCluster(skuId: Long, dpClusterId: Long) : Future[ClusterSku]
    def disableSkuForCluster(skuId: Long, dpClusterId: Long) : Future[Long]
  }
