...
class DpClusterServiceImpl(config: Config)(implicit ws: WSClient)
  ...
  override def delete(dpClusterName: String, dpDatacenter: String): Future[Either[Errors, Boolean]] = {
    ws.url(s"$url/dp/clusters?clusterName=$dpClusterName&datacenterName=$dpDatacenter")
      .withHeaders("Accept" -> "application/json")
      .delete()
      .map(mapStatus)
  }
